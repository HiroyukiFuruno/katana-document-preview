...
- CI run `30878695303`, Windows job `91895238013`, passed AppContainer acceptance, all 1675 KDV unit tests, and all 95 KDV linter unit tests, then exposed the remaining workspace AST integration path boundary. The length-debt baseline now derives one normalized relative path from the workspace root, including mixed Windows separators, while rejecting root-prefix collisions.
- CI run `30881147000`, Windows job `91902623148`, passed AppContainer acceptance and all workspace tests, then exposed two Storybook assertions that compared the normalized public document ID against Rust's Windows-only verbatim `canonicalize()` spelling. The source contract now verifies filesystem identity after canonicalization and separately rejects a leaked `//?/` prefix, without weakening direct-image URI coverage.
