...
- [/] 6.16 KDV `v0.5.4`のexternal hyperlink表示修正について、strict coverage 100% / uncovered 0、3 OS CI、package、publish dry-run、GitHub Release、crates.io publicationを確認する。delegation-exception: `直列のクリティカルパス` / 証跡: command: `rtk just VERSION=0.5.4 release-check` success / functions `3024/3024` / lines `24753/24753` / uncovered `0` / `multi_format_office_preflight_contract` `10/10` / `multi_format_office_worker_contract` `10/10` / package success / publish dry-run success / 残件: 3 OS CI、GitHub Release、crates.io publication
- [x] 6.17 Rust stable `1.98`の`clippy::chunks_exact_to_as_chunks`追加でCI lintが拒否したproduction / 契約test / Storybookの全固定幅pixel走査を、末尾を無視する従来挙動のまま`as_chunks`へ置換する。delegation-exception: `直列のクリティカルパス` / 証跡: CI run `32452506376` macOS / Ubuntu lint failure、CI run `32454315323` Ubuntu integration-test lint failure、CI run `32456030838` Ubuntu Storybook lint failure / command: `rtk rg 'chunks_exact(_mut)?\\(([^)]*)\\)' --glob '*.rs' --glob '!target/**'` result `0` / command: `rtk just lint` success / command: `rtk just VERSION=0.5.4 release-check` success, functions `3024/3024`, lines `24753/24753`, uncovered `0`
