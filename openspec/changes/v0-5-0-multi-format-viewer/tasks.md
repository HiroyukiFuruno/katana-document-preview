...
- [x] 6.17 Rust stable `1.98`の`clippy::chunks_exact_to_as_chunks`追加でCI lintが拒否したproduction / testの全固定幅pixel走査を、末尾を無視する従来挙動のまま`as_chunks`へ置換する。delegation-exception: `直列のクリティカルパス` / 証跡: CI run `32452506376` macOS / Ubuntu lint failure、CI run `32454315323` Ubuntu integration-test lint failure / command: `rtk rg 'chunks_exact(_mut)?\\(([^)]*)\\)' crates --glob '*.rs'` result `0` / command: `rtk just lint` success / test: `multi_format_pdf_contract` `5 passed`, `diagram_svg_artifact` `8 passed` / command: `rtk just VERSION=0.5.4 release-check` success, functions `3024/3024`, lines `24753/24753`, uncovered `0`
