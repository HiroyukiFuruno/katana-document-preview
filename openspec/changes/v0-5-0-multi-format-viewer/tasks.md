...
- [/] 4.20 `office2pdf`のPowerPoint paragraph line metricsがrenderingと同じfont fallback chainを使う上流修正を採用する。delegation-exception: `ユーザーがsubagent利用を禁止` / 証跡: upstream issue `#705` / PR `#745` / merge commit `c528eef467aaf9ca4873acf5c8bedb07b7ae5596` / upstream 3 OSを含む17 checks success / temporary exact git pin and local `multi_format_office_worker_contract` 9 tests success / command: `rtk just coverage` functions `3020/3020` lines `24712/24712` uncovered functions/lines `0` / release request `https://github.com/developer0hye/office2pdf/issues/959`。crates.io latest `0.6.5`には未収録のため、既存release guardが拒否するgit pinは3 OS検証専用とし、registry patch releaseへ戻すまでKDVを公開しない。fork crate、KDV内のOOXML書換え・独自font substitution parser・独自layout補正で迂回しない / 残件: upstream patch release、registry pin、3 OS release gate
