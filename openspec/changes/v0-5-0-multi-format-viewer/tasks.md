...
- [/] 6.14 deterministic Office font修正を含むKDV `v0.5.2`について、strict coverage 100% / uncovered 0、3 OS CI、package、publish dry-run、GitHub Release、crates.io publicationを確認する。delegation-exception: `ユーザーがsubagent利用を禁止` / ローカル検証: command: `rtk just VERSION=0.5.2 release-check` success / functions `3021/3021` / lines `24728/24728` / uncovered functions `0` / uncovered lines `0` / package success / publish dry-run success / `office2pdf-katana 0.6.6` registry source・checksum・supply-chain evidence確認済み / 残件: 3 OS CI、GitHub Release、crates.io publication
