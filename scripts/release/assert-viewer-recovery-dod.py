...
def self_test() -> int:
    ...
    if kuc_cargo_dependency_errors(
        ...
        failures.append("KUC Cargo dependency scanner must allow exact registry v0.3.5 deps")
    if not kuc_cargo_dependency_errors(
        valid_kuc_cargo_toml.replace('version = "=0.3.5"', 'version = "=0.3.4"'),
        valid_kuc_cargo_lock,
        "mod document_viewer;\n",
    ):
    ...
    if not kuc_cargo_dependency_errors(
        valid_kuc_cargo_toml,
        valid_kuc_cargo_lock.replace('version = "0.3.5"', 'version = "0.3.4"'),
        "mod document_viewer;\n",
    ):
        failures.append("KUC Cargo dependency scanner must reject stale Cargo.lock version")
    if not kuc_cargo_dependency_errors(
        valid_kuc_cargo_toml.replace(
            'version = "=0.3.5", features',
            'git = "https://github.com/HiroyukiFuruno/katana-ui-core.git", version = "=0.3.5", features',
        ),
        valid_kuc_cargo_lock,
        "mod document_viewer;\n",
    ):
    ...
    if not kuc_cargo_dependency_errors(
        valid_kuc_cargo_toml
        + '\nkatana-ui-core-storybook = { package = "katana-ui-core", version = "=0.3.5", features = ["raster-host"] }',
        valid_kuc_cargo_lock,
        "mod document_viewer;\n",
    ):
